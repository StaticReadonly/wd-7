  ...
  methods: {
    ...
    async addTag(tag) {
      if (tag && !this.newRecipe.tags.some(t => t.name === tag.name)) {
        ...
        try {
          ...
          const data = await response.json();
          this.newRecipe.tags.push(tag);
          this.tags.allTags.push(data.id);
        } catch (error) {
    ...
    removeIngredient(index) {
      if (index !== -1) {
        ...
        this.ingredients.allIngredients.splice(index, 1);
      }
    },
    ...
    async addRecipe() {
      try {
        ...
        if (response.ok) {
          ...
          this.tags = { allTags: [], filteredTags: [], tagSearch: ''};
          this.ingredients = { allIngredients: [], filteredIngredients: [], ingredientSearch: ''};
        }
    ...
    updateProportion(updatedIngredient) {
    const ingredientIndex = this.newRecipe.ingredients.findIndex(
      ing => ing.id === updatedIngredient.id
    );
    if (ingredientIndex !== -1) {
      this.ingredients.allIngredients[ingredientIndex] = { ...updatedIngredient };
    }
  },
  },
};
  ...
  <div class="recipe">
    ...
    <form @submit.prevent="addRecipe">
      ...
      <IngredientsManagement
        ...
        @update-proportion="updateProportion"
        />
