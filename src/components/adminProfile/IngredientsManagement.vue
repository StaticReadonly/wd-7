...
export default {
  ...
  methods: {
    ...
    selectIngredient(event) {
      ...
      if (selectedIngredient && !this.ingredients.some((ing) => ing.name === selectedName)) {
    ...
    addIngredient() {
      ...
      if (newIngredient && !this.ingredients.some((ing) => ing.name === newIngredient)) {
        const newIngredientData = { name: newIngredient, unit: newIngredientUnit };
        this.$emit('add-ingredient', { ...newIngredientData, proportion: '' });

        // Виклик бекенду для створення нового інгредієнта
...
<template>
  <div>
    ...
    <div>
      ...
      <button type="button" @click="searchQuery">Пошук</button>
      ...
      <select v-if="filteredIngredients.length > 0" @change="selectIngredient">
        ...
        <option v-for="ingredient in filteredIngredients" :key="ingredient.id" :value="ingredient.name">
          {{ ingredient.name }} ({{ ingredient.unit }})
        </option>
      </select>
      <p v-else>Немає доступних інгредієнтів</p>
    </div>
