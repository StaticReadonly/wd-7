...
export default {
  ...
  methods: {
    ...
    updateProportion(index) {
      const updatedIngredient = this.ingredients[index];
      this.$emit('update-proportion', { ...updatedIngredient });
    },
  },
};
</script>
...
<template>
  <div>
    ...
    <h4>Створити новий інгредієнт:</h4>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(ingredient, index) in ingredients" :key="ingredient.id">
          ...
          <td>
            <input v-model="ingredient.proportion" type="text" @input="updateProportion(index)" placeholder="Пропорція"/>
          </td>
